...
export class AwsS3Service implements IAwsS3Service {
    ...
    async putItemInBucketWithAcl(
        ...
        // Determine the MIME type (consider using a library for accuracy)
        const mime: string = file.originalname.substring(
            file.originalname.lastIndexOf('.') + 1
        );
        const filename = options?.customFilename
        ...
        // Construct the key without leading slashes
        ...
        try {
            ...
            // Ensure there's only one slash between baseUrl and key
            const completedUrl = `${this.baseUrl}${this.baseUrl.endsWith('/') ? '' : '/'}${key}`;
