...
export class RoleService implements IRoleService {
    ...
    async findOneByName(
        ...
        const role = await this.roleRepository.findOne({ name }, options);
        if (!role) {
    ...
    async findAllByTypes(
        ...
        return this.roleRepository.findAll({ type: { $in: types } }, options);
    }
    ...
    async findOneByType(
        ...
        const role = await this.roleRepository.findOne({ type }, options);
        if (!role) {
            throw new NotFoundException({
                statusCode: ENUM_ROLE_STATUS_CODE_ERROR.NOT_FOUND,
                message: 'role.error.notFound',
