...
import { CountrySuggestionListResponseDto } from 'src/modules/country/dtos/response/country-suggestion.list.response.dto';
...
export function CountrySuggestionsAdminListDoc(): MethodDecorator {
    return applyDecorators(
        Doc({
            summary: 'get all of countries suggestions',
        }),
        DocAuth({
            jwtAccessToken: true,
        }),
        DocGuard({ role: true, policy: true }),
        DocResponsePaging<CountrySuggestionListResponseDto>(
            'countrySuggestion.list',
            {
                dto: CountrySuggestionListResponseDto,
            }
        )
    );
}
