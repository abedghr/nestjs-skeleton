...
export function Doc(options?: IDocOptions): MethodDecorator {
    ...
    if (options?.extra?.location != null) {
        headers.push({
            name: 'x-latitude',
            description: 'Latitude header',
            required: false,
            schema: {
        ...
        headers.push({
            ...
            required: true,
            schema: {
                default: faker.location.countryCode(),
...
export function DocRequest(options?: IDocRequestOptions) {
    ...
    if (options?.dto && options?.isArray) {
        docs.push(ApiBody({ type: options?.dto, isArray: true }));
    } else if (options?.dto) {
...
export function DocResponsePaging<T>(
    ...
    return applyDecorators(
        ...
        ApiQuery({
            ...
            type: 'string',
            description:
                'Text Search will base on _metadata.pagination._availableSearch with rule contains, and case insensitive',
        }),
        ApiQuery({
            name: 'search',
            required: false,
