...
export class HelperStringService implements IHelperStringService {
    ...
    splitPoint(coordinate: string): number[] {
        const pointArr: number[] = coordinate
            .split(',')
            .map(str => Number(str));
        return pointArr;
    }
    checkPoint(coordinate: string): boolean {
        const pointArr: string[] = coordinate.split(',');
        return pointArr.length == 2;
    }
}
