...
export class PaginationService implements IPaginationService {
    ...
    textSearch(
        ...
        if (
            !textSearchValue ||
            textSearchValue === '' ||
            availableTextSearch.length === 0
        ) {
            return undefined;
        }

        // Return a single `$text` search that will search across all indexed fields
        return DatabaseQueryTextSearch(textSearchValue);
    }
