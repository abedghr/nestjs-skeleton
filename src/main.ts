...
const PATH_TO_KEY =
    env === ENUM_APP_ENVIRONMENT.PRODUCTION
        ? path.join(__dirname, 'secrets/private-prod.key')
        : path.join(__dirname, 'secrets/private.key');

const PATH_TO_CERT =
    env === ENUM_APP_ENVIRONMENT.PRODUCTION
        ? path.join(__dirname, 'secrets/certificate-prod.crt')
        : path.join(__dirname, 'secrets/certificate.pem');

async function bootstrap() {
    const app: NestApplication = await NestFactory.create(AppModule, {
        snapshot: true,
        httpsOptions:
            process.env.APP_ENV === ENUM_APP_ENVIRONMENT.LOCAL
                ? undefined
                : {
    ...
    const host: string =
        configService.get<string>('app.http.host') ?? 'localhost';
    const port: number =
