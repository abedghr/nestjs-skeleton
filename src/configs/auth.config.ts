...
export default registerAs(
    ...
    (): Record<string, any> => ({
        ...
        ngt: {
            name: process.env.NGT_SERVICE_USERNAME,
            password: process.env.NGT_SERVICE_PASSWORD,
            sender: process.env.SMS_SERVICE_SENDER,
        },

        testingUsers:
            process.env.TESTING_USERS != null
                ? process.env.TESTING_USERS.split(',')
                : [],

        testingOtp:
            process.env.TESTING_OTP != null
                ? process.env.TESTING_OTP
                : '345678',
    })
);
